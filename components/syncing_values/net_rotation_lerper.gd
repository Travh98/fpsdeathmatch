...
## node's networked_rotation variable.
...
@export var networked_rotation: Vector3
...
func _physics_process(delta):
	...
	if not is_multiplayer_authority(): 
		controlled_object.global_rotation.x = lerp_angle(controlled_object.global_rotation.x, networked_rotation.x, lerp_speed * delta)
		controlled_object.global_rotation.y = lerp_angle(controlled_object.global_rotation.y, networked_rotation.y, lerp_speed * delta)
		controlled_object.global_rotation.z = lerp_angle(controlled_object.global_rotation.z, networked_rotation.z, lerp_speed * delta)
		return
	
	# Host will update the networked rotation
	networked_rotation = controlled_object.global_rotation
